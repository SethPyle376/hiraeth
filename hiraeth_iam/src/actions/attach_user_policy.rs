...
mod tests {
    ...
    use super::AttachUserPolicyAction;
    ...
    #[tokio::test]
    async fn handle_attaches_existing_policy_to_user() {
        let action = TypedAwsActionAdapter::new(AttachUserPolicyAction);
        let store = store();
        let response = action
            .handle(
                resolved_request(
        ...
        assert_eq!(attached[0].policy_name, "orders-readonly");
        assert_eq!(attached[0].policy_path.as_deref(), Some("/dev/"));
    }
